...
const repoNameEl = document.getElementById("repoNameHeader");
...
async function fetchRepoTree() {
  ...
  repoNameEl.textContent = CONFIG.repo;
...
function renderTree(nodes, container, filter = "") {
  ...
  function renderNode(node, parentEl) {
    ...
    const toggleEl = document.createElement("div");
    toggleEl.className = "tree-toggle";
    if (node.type === "dir") toggleEl.textContent = "▾";
...
async function selectFile(path) {
  ...
  if (!path) {
    currentPathEl.innerHTML =
      `<a class="rru-link" href="${pagesURL}" target="_blank" rel="noopener noreferrer">/${CONFIG.repo}</a>`;
  } else {
    currentPathEl.innerHTML =
      `<a class="rru-link" href="${pagesURL}" target="_blank" rel="noopener noreferrer">/${path}</a>`;
  }
  ...
  const activeNode = document.querySelector(
    `.tree-node[data-path="${CSS.escape(path)}"]`
  );
  ...
  const rawURL =
    `${CONFIG.raw_base}/${CONFIG.owner}/${CONFIG.repo}/${CONFIG.branch}/${rawPathEncoded}`;

  if (isImageFile(path)) {
    fileTypeBadgeEl.textContent = "Image";
    fileContentEl.innerHTML =
      `<img src="${rawURL}" style="max-width:100%;border-radius:6px;">`;
  ...
  fileTypeBadgeEl.textContent =
    isTextFile(path) ? "Text file" : "Plain text (unsupported ext)";
  fileMetaEl.textContent =
    `${file.size || 0} bytes • sha ${file.sha.slice(0, 7)}`;
  ...
  try {
    ...
    emptyStateEl.style.display = "flex";
    emptyStateEl.innerHTML =
      `<div>Failed to load file.<br/><span style="font-size:11px;color:#ff4b6a;">${err.message}</span></div>`;
...
function collapseAllDropdowns() {
  ...
  document.querySelectorAll(".tree-toggle").forEach(el => {
    if (!el.classList.contains("hidden")) el.textContent = "▸";
  });
}
